...
public class ArrayHashQuestion1 {
   ...
   * <ul>
   * <li>哈希专题：补数 / 映射关系</li>
   * <li>特征：遍历当前元素时，需要快速查找它的“搭档”是否已经出现</li>
   * </ul>
   *
   * <p>思路：
   *
   * <ol>
   * <li>用哈希表记录“值 -> 下标”。</li>
   * <li>遍历当前元素时，先计算补数 {@code target - nums[i]}。</li>
   * <li>如果补数已经出现，就直接返回答案；否则把当前值放进哈希表。</li>
   * </ol>
   *
   * <p>优点：
   *
   * <ul>
   * <li>时间复杂度从暴力的 {@code O(n^2)} 降到 {@code O(n)}。</li>
   * <li>“先查再放”这个细节非常适合训练哈希补数题的标准节奏。</li>
   * </ul>
   *
   * <p>缺点：
   *
   * <ul>
   * <li>需要额外的哈希表空间。</li>
   * <li>如果更新顺序写反，像 `[3,3]` 这类用例就容易把同一个元素重复使用。</li>
   * </ul>
   *
   * <p>变体应对：
   *
   * <ul>
   * <li>如果数组已经有序，可以改成双指针做法，把空间降到 {@code O(1)}。</li>
   * <li>如果题目要求返回所有组合或更多元组，通常需要排序加双指针而不是继续只用哈希表。</li>
   * </ul>
   */
  public int[] twoSum(int[] nums, int target) {
